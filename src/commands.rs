...
pub(crate) async fn ai(
    ...
    #[description = "Ollama model e.g. llama3.1, or gemini-3.6-flash for Gemini"] model: Option<String>,
    ...
    if !changing {
        ...
        post_text(ctx, format!(
            "AI chat is **{state}** — model `{ai_model}` via {backend} (`{ai_host}`).\nGemini key: {key_state} (`gemini_api_key` in config or GEMINI_API_KEY env).\n{web}\nOwner: `/ai true model:llama3.1` for local, `/ai true model:gemini-3.6-flash` for Gemini. Then just ping me `@artixy <question>` or `artixy <question>`.",
        ))
        .await?;
        return Ok(());
    }
